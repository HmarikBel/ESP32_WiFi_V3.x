		...
		rapiSender.sendCmd("$G5", [](int ret) {
			if (RAPI_RESPONSE_OK == ret)
			{
				if(rapiSender.getTokenCnt() >= 8)
				{
					...
					Data.m_temperatureIn.m_currentValue = strtol(rapiSender.getToken(6), NULL, 10) / 10.0;
					Data.m_temperatureInMax = strtol(rapiSender.getToken(7), NULL, 10) / 10.0;
				} 
			}
			else {
				...
				Data.m_temperatureIn.m_currentValue = -1;
				Data.m_temperatureInMax = -1;
			}
			...
			WiFiLogic.SendIfNecessary(COUNTER_ID_CHARGER_TEMPERATURE_IN, &Data.m_temperatureIn, 1, SEND_DATA_MIN, SEND_DATA_MAX);
		});
	}
}
